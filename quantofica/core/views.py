...
def home(request):
    ...
    if 'currency' in request.GET and 'value' in request.GET:
        form = NubankExchangeRateForm(request.GET)
        if form.is_valid():
            output = form.calculate()
        default_currency = form.cleaned_data.get('currency', default_currency)
    else:
        form = NubankExchangeRateForm(initial={'currency': default_currency})
